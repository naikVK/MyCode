...
export default {
  ...
  data () {
    return {
      ...
      username: '',
      checked: false,
      showEmail: JSON.parse(JSON.stringify(this.$store.state.config.purpose.settings.otprequired.type_otp.email))
    }
  },
  methods: {
        ...
        axios.post(`o/login/validateusername`, { 'username': username })
           .then(Response => {
             ...
             if (Response.status === 200) {
               forgotpassword.$store.commit('SET_USERNAME', this.result)
      ...
      if (this.showEmail) {
        ...
        .then(Response => {
          ...
          if (Response.status === 200) {
            ...
            var mypath = '/forgotpasswordotpverification/'
